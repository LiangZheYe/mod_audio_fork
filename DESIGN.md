# mod_audio_fork 详细设计文档

## 1. 模块概述

mod_audio_fork 是一个 FreeSWITCH 可加载模块，利用 FreeSWITCH 的 Media Bug 机制将通话音频实时通过 WebSocket 流式传输到远端服务器，并支持从远端服务器接收音频/指令注入通话。典型应用场景包括实时语音转写、AI 语音助手、双向语音流处理等。

### 1.1 核心能力

| 能力 | 说明 |
|------|------|
| 音频 Fork (上行) | 将通话的 read 侧音频流式发送到远端 WebSocket 服务器 |
...
| JSON 信令 | 支持 playAudio / killAudio / mark / clearMarks 等控制信令 |
| Mark 机制 | 在音频流中插入标记点，用于同步播放进度 |
...
| Flag | 含义 | 用途 |
|------|------|------|
| `SMBF_READ_STREAM` | 监听 read 侧（远端→本端）音频 | 上行 fork：将对方语音发送到服务器 |
...
capture_callback(bug, user_data, type)
    ...
    │
    ├── SWITCH_ABC_TYPE_CLOSE ─────────→ fork_session_cleanup(session, bugname, NULL, 1)
    │                                    │  channelIsClosing=1 表示由 FreeSWITCH 驱动关闭
    │
    ├── SWITCH_ABC_TYPE_READ ──────────→ fork_frame(session, bug)
    │                                    │  每 20ms 触发一次
...
因此本模块采用"零拷贝 + 锁保护"策略：`fork_frame` 使用 `trylock`（非阻塞），`dub_speech_frame` 使用阻塞锁但操作极简（仅 buffer 读写）。
...
└──────────┬──────────────────────────────┬──────────────────────┘
           │ capture_callback              │ fork_function (API)
           │ (media 线程回调)               │ (API 线程)
           │                              │
┌──────────▼──────────────────────────────▼──────────────────────┐
│                      mod_audio_fork.c                           │
│  ┌──────────────────┐  ┌───────────────────────────────────┐   │
│  │ capture_callback │  │ fork_function (API 入口)          │   │
│  │   fork_frame()   │  │   start_capture() / do_stop()    │   │
│  │   dub_speech_    │  │   send_text() / do_pauseresume() │   │
...
│                       lws_glue.cpp (C++ 桥接层)                │
│  ┌──────────────┐  ┌──────────────┐  ┌────────────────────┐   │
│  │ fork_frame   │  │ dub_speech_  │  │ processIncoming    │   │
│  │ (上行:音频    │  │ frame       │  │ Message/Binary     │   │
│  │  写入buffer) │  │ (下行:从     │  │ (下行:WS消息解析   │   │
│  │              │  │  buffer读取) │  │  与业务逻辑处理)   │   │
│  └──────┬───────┘  └──────┬───────┘  └────────┬───────────┘   │
│         │                 │                    │               │
│  ┌──────▼─────────────────▼────────────────────▼───────────┐   │
...
│  │  · m_metadata_list (待发送文本队列)                       │   │
│  │  · m_recv_buf (接收消息重组缓冲区)                       │   │
...
| 文件 | 语言 | 职责 |
|------|------|------|
| `mod_audio_fork.c` | C | FreeSWITCH 模块入口，API 命令解析，media bug 回调分发 |
| `mod_audio_fork.h` | C | 数据结构定义（private_t），常量宏，事件类型 |
| `lws_glue.cpp` | C++ | C/C++ 桥接层，上行音频采集、下行音频注入、信令处理、会话生命周期 |
| `audio_pipe.cpp` | C++ | WebSocket 传输层，LWS 回调处理，连接管理，数据收发 |
| `audio_pipe.hpp` | C++ | AudioPipe 类定义，状态枚举，回调类型 |
| `parser.cpp` | C++ | JSON 消息解析 |
| `vector_math.cpp` | C | SIMD 优化的向量运算（加法、归一化、音量调节） |
| `base64.hpp` | C++ | Base64 编解码（header-only） |
...
│  │              │  │             │  │              │            │
│  │ fork_function│  │capture_     │  │capture_      │            │
...
- **触发方式**: ESL / CLI / 事件套接字调用 `uuid_audio_fork` 命令
- **执行函数**: `fork_function()` → `start_capture()` / `do_stop()` / `send_text()` 等
- **关键操作**:
  - `switch_core_session_locate(argv[0])` 获取 session 引用（引用计数+1）
  - 调用 `fork_session_init()` 分配 private_t 和 AudioPipe
...
- **上行路径 (SWITCH_ABC_TYPE_READ → fork_frame)**:
...
- **启动时机**: 模块加载时 `fork_init()` → `AudioPipe::initialize()` → `std::thread`
...
| `m_text_mutex` | `m_metadata_list` | 待发送文本队列保护 |
...
### 5.1 上行数据流 (Fork: 通话 → 服务器)

```
  RTP        Media Bug      fork_frame()        AudioPipe          LWS          WebSocket
...
2. `fork_frame()` 获取 `tech_pvt->mutex` (trylock) 和 `m_audio_mutex`
...
#### 路径 A: JSON playAudio 信令 (传统方式)

```
  Server ──→ WebSocket ──→ LWS_RECEIVE ──→ eventCallback ──→ processIncomingMessage
                                                                          │
                                                              base64_decode(audioContent)
                                                                          │
                                                              switch_mutex_lock(tech_pvt->mutex)
                                                                          │
                                                              playoutBuffer.insert(decoded_audio)
                                                                          │
                                                              switch_mutex_unlock
```
...
Mark 机制用于在音频流中标记关键时间点（如 TTS 分句边界），实现播放进度同步：

```
  服务器发送:                    客户端接收:
  ──────────                    ──────────
  {"type":"mark",               收到 mark → 放入 pVecMarksInInventory
   "data":{"name":"m1"}}        │
                                下一段二进制音频到达:
  [binary audio data]           │ mark 从 Inventory 移到 InUse
                                │ 在 preBuffer 前插入 AUDIO_MARKER (0xFFFF)
                                │
                                dub_speech_frame 消费到 AUDIO_MARKER:
                                │ 取出 pVecMarksInUse.front()
                                │ 发送 mark event 到服务器
                                │ 替换 AUDIO_MARKER 为 0 (静音)
```
...
| 资源 | 分配方式 | 分配者 | 释放者 |
|------|---------|--------|--------|
| `private_t` | `switch_core_session_alloc()` | `fork_session_init` | FreeSWITCH session pool 自动释放 |
| `AudioPipe` | `new` | `fork_data_init` | LWS 线程 `delete` |
| `m_audio_buffer` | `new uint8_t[]` | AudioPipe 构造 | AudioPipe 析构 |
| `CircularBuffer_t` (playout) | `new` | `fork_data_init` | `destroy_tech_pvt` |
| `CircularBuffer_t` (prebuffer) | `new` | `fork_data_init` | `destroy_tech_pvt` |
| `SpeexResamplerState` | `speex_resampler_init()` | `fork_data_init` | `destroy_tech_pvt` |
| `switch_mutex_t` | `switch_mutex_init()` | `fork_data_init` | `destroy_tech_pvt` |
| `m_recv_buf` | `malloc/realloc` | LWS RECEIVE 回调 | AudioPipe 析构 |
| `pVecMarks*` | `new deque<string>` | 首次 mark 消息 | `destroy_tech_pvt` |
...
| 路径 | 操作 | 预估耗时 | 备注 |
|------|------|---------|------|
| 上行: fork_frame | trylock + bug_read + unlock | **5-20 μs** | 20ms 周期内最优路径 |
| 上行: fork_frame (重采样) | lock + bug_read + speex_resample + unlock | **30-80 μs** | Speex 重采样开销 |
...
| 持有者 | 持有时长 | 频率 | 阻塞影响 |
|--------|---------|------|---------|
| fork_frame (media线程) | 5-80 μs | 50次/秒 | trylock 失败则丢帧 |
| dub_speech_frame (media线程) | 3-15 μs | 50次/秒 | 阻塞锁，会影响 fork_frame |
| processIncomingBinary (LWS线程) | 20-60 μs | 取决于服务器 | 与 media 线程竞争 |
| processIncomingMessage (LWS线程) | 5-30 μs | 取决于服务器 | 与 media 线程竞争 |
| fork_session_cleanup (API线程) | 10-50 μs | 1次/通话 | 会阻塞 media 线程 |

**风险评估**:
- `fork_frame` 使用 `trylock`，竞争时丢弃当前帧（可接受，20ms 后重试）
- `dub_speech_frame` 使用阻塞锁，但持锁时间极短（<15μs），与 LWS 线程竞争窗口小
- 最大风险: `fork_session_cleanup` 持锁期间如果同时有多个 LWS 回调，可能造成短暂阻塞
...
| 持有者 | 持有时长 | 频率 |
|--------|---------|------|
| fork_frame (media线程) | 5-80 μs | 50次/秒 |
...
| 组件 | 大小 |
|------|------|
| private_t | ~8.5 KB (含 initialMetadata[8192]) |
...
| 瓶颈 | 影响 | 优化建议 |
|------|------|---------|
| `trylock` 丢帧 | 高负载时上行音频丢失 | 增大 audio buffer（MOD_AUDIO_FORK_BUFFER_SECS） |
...
| 环境变量 | 默认值 | 范围 | 说明 |
|---------|--------|------|------|
| `MOD_AUDIO_FORK_BUFFER_SECS` | 2 | 1-5 | 上行音频缓冲区大小（秒） |
| `MOD_AUDIO_FORK_TCP_KEEPALIVE_SECS` | 55 | - | TCP keepalive 间隔（秒） |
| `MOD_AUDIO_FORK_SUBPROTOCOL_NAME` | `audio.drachtio.org` | - | WebSocket 子协议名称 |
| `MOD_AUDIO_FORK_SERVICE_THREADS` | 1 | 1-5 | LWS service 线程数（当前未启用多线程） |
| `MOD_AUDIO_FORK_HTTP_AUTH_USER` | (空) | - | 全局 HTTP Basic Auth 用户名 |
| `MOD_AUDIO_FORK_HTTP_AUTH_PASSWORD` | (空) | - | 全局 HTTP Basic Auth 密码 |
...
| `MOD_AUDIO_FORK_ALLOW_SELFSIGNED` | 允许自签名 TLS 证书 |
| `MOD_AUDIO_FORK_SKIP_SERVER_CERT_HOSTNAME_CHECK` | 跳过 TLS 证书主机名验证 |
| `MOD_AUDIO_FORK_ALLOW_EXPIRED` | 允许过期 TLS 证书 |

### 9.3 API 命令

```
uuid_audio_fork <uuid> start <wss-url> [mono|mixed|stereo] [8k|16k|24k|32k|64k] \
  [bugname] [metadata] [bidirectionalAudio_enabled] [bidirectionalAudio_stream_enabled] \
...
| 事件子类 | 触发时机 | 载荷 |
|---------|---------|------|
| `mod_audio_fork::connect` | WebSocket 连接成功 | 无 |
| `mod_audio_fork::connect_failed` | WebSocket 连接失败 | `{"reason":"..."}` |
| `mod_audio_fork::disconnect` | 连接被远端关闭 | 无 |
| `mod_audio_fork::transcription` | 收到 transcription 消息 | JSON data |
| `mod_audio_fork::transfer` | 收到 transfer 消息 | JSON data |
| `mod_audio_fork::play_audio` | 收到 playAudio 消息 | JSON data |
| `mod_audio_fork::kill_audio` | 收到 killAudio 消息 | 无 |
| `mod_audio_fork::error` | 收到 error 消息 | JSON data |
| `mod_audio_fork::buffer_overrun` | 上行缓冲区溢出 | 无 |
| `mod_audio_fork::json` | 收到未知类型 JSON | 完整 JSON |
