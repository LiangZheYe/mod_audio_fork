...
class AudioPipe {
 public:
  ...
  static void initialize(const char* protocolName, int loglevel,
                         log_emit_function logger);
  static bool deinitialize();
  static bool lws_service_thread();

  // constructor
  AudioPipe(const char* uuid, const char* host, unsigned int port,
            const char* path, int sslFlags, size_t bufLen, size_t minFreespace,
            const char* username, const char* password, char* bugname,
            int bidirectional_audio, notifyHandler_t callback);
  ~AudioPipe();

  LwsState_t getLwsState(void) {
    return m_state.load();
  }  // BUG-20 fix: m_state is now atomic
  void connect(void);
  size_t binarySpaceAvailable(void) {
    return m_audio_buffer_max_len - m_audio_buffer_write_offset;
  }
  size_t binaryMinSpace(void) { return m_audio_buffer_min_freespace; }
  char* binaryWritePtr(void) {
    return (char*)m_audio_buffer + m_audio_buffer_write_offset;
  }
  void binaryWritePtrAdd(size_t len) { m_audio_buffer_write_offset += len; }
  void binaryWritePtrResetToZero(void) { m_audio_buffer_write_offset = 0; }
  void lockAudioBuffer(void) { m_audio_mutex.lock(); }
  void unlockAudioBuffer(void);
  bool hasBasicAuth(void) { return !m_username.empty() && !m_password.empty(); }
  ...
  bool isGracefulShutdown(void) { return m_gracefulShutdown; }

  bool is_bidirectional_audio_stream() { return m_bidirectional_audio_stream; }
 ...
 private:
  ...
  std::atomic<LwsState_t>
      m_state;  // BUG-20 fix: use atomic to prevent data race across threads
  std::string m_uuid;
  std::string m_host;
