...
#define FRAME_SIZE_8000 \
  320 /*which means each 20ms frame as 320 bytes at 8 khz (1 channel only)*/
...
static unsigned int nServiceThreads __attribute__((unused)) = std::max(
    1, std::min(
           requestedNumServiceThreads ? ::atoi(requestedNumServiceThreads) : 1,
           5));
static unsigned int idxCallCount = 0;

switch_status_t processIncomingBinary(private_t *tech_pvt,
                                      switch_core_session_t * /*session*/,
                                      const char *message, size_t dataLength) {
  ...
  CircularBuffer_t *cBuffer =
      static_cast<CircularBuffer_t *>(tech_pvt->streamingPreBuffer);

  // Ensure the prebuffer has enough capacity
  if (cBuffer->capacity() - cBuffer->size() < numSamples) {
    size_t newCapacity =
        cBuffer->size() + std::max(numSamples, (size_t)BUFFER_GROW_SIZE);
  ...
  // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Appended %zu
  // 16-bit samples to the prebuffer.\n", numSamples);

  // if we haven't reached threshold amount of prebuffered data, return
  if ((int)cBuffer->size() < tech_pvt->streamingPreBufSize) {
    // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Prebuffered data
    // is below threshold %u, returning.\n", tech_pvt->streamingPreBufSize);
    return SWITCH_STATUS_SUCCESS;
  }

  // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Prebuffered data
  // samples %u is above threshold %u, prepare to playout.\n", cBuffer->size(),
  // tech_pvt->streamingPreBufSize);
  ...
  // Calculate the number of samples that can be evenly divided by the
  // downsample factor
  size_t numCompleteSamples =
      (cBuffer->size() / downsample_factor) * downsample_factor;
  ...
  if (numLeftoverSamples > 0) {
    ...
    // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Temporarily
    // removing %u leftover samples due to downsampling.\n",
    // numLeftoverSamples);
  }
  ...
  try {
    if (tech_pvt->bidirectional_audio_resampler) {
      // Improvement: Use assign to convert circular buffer to vector for
      // resampling
      ...
      // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Resampling %u
      // samples into a buffer that can hold %u samples\n", in.size(), out_len);

      speex_resampler_process_interleaved_int(
          tech_pvt->bidirectional_audio_resampler, in.data(), &in_len,
          out.data(), &out_len);

      // Resize the output buffer to match the output length from resampler
      // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Resizing
      // output buffer from %u to %u samples\n", in.size(), out_len);

      out.resize(out_len);
    } else {
      // If no resampling is needed, copy the data from the prebuffer to the
      // output buffer
      out.assign(cBuffer->begin(), cBuffer->end());
    }
  } catch (const std::exception &e) {
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                      "Error resampling incoming binary message: %s\n",
                      e.what());
    return SWITCH_STATUS_FALSE;
  } catch (...) {
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                      "Error resampling incoming binary message\n");
    return SWITCH_STATUS_FALSE;
  }

  if (nullptr != tech_pvt->mutex &&
      switch_mutex_lock(tech_pvt->mutex) == SWITCH_STATUS_SUCCESS) {
    CircularBuffer_t *playoutBuffer =
        (CircularBuffer_t *)tech_pvt->streamingPlayoutBuffer;

    try {
      // Resize the buffer if necessary
      if (playoutBuffer->capacity() - playoutBuffer->size() < out.size()) {
        size_t newCapacity = playoutBuffer->size() +
                             std::max(out.size(), (size_t)BUFFER_GROW_SIZE);
        playoutBuffer->set_capacity(newCapacity);
        // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Resized
        // playout buffer to new capacity: %zu\n", newCapacity);
      }
      // Push the data into the buffer.
      playoutBuffer->insert(playoutBuffer->end(), out.begin(), out.end());
      // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Appended %zu
      // 16-bit samples to the playout buffer.\n", out.size());
    } catch (const std::exception &e) {
      switch_mutex_unlock(tech_pvt->mutex);
      cBuffer->clear();
      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                        "Error processing incoming binary message: %s\n",
                        e.what());
      return SWITCH_STATUS_FALSE;
    } catch (...) {
      switch_mutex_unlock(tech_pvt->mutex);
      cBuffer->clear();
      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                        "Error processing incoming binary message\n");
      return SWITCH_STATUS_FALSE;
    }
    switch_mutex_unlock(tech_pvt->mutex);
    cBuffer->clear();
    ...
    if (!leftoverSamples.empty()) {
      cBuffer->insert(cBuffer->end(), leftoverSamples.begin(),
                      leftoverSamples.end());
      // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Put back %u
      // leftover samples into the prebuffer.\n", leftoverSamples.size());
    }
    return SWITCH_STATUS_SUCCESS;
  }
  switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                    "Failed to get mutext (temp)\n");

  return SWITCH_STATUS_SUCCESS;
}

static void eventCallback(const char *sessionId, const char *bugname,
                          drachtio::AudioPipe::NotifyEvent_t event,
                          const char *message, const char *binary, size_t len) {
        ...
        switch (event) {
          case drachtio::AudioPipe::CONNECT_SUCCESS:
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                              SWITCH_LOG_INFO, "connection successful\n");
          ...
          case drachtio::AudioPipe::CONNECT_FAIL: {
            ...
            tech_pvt->responseHandler(session, EVENT_CONNECT_FAIL,
                                      (char *)json.str().c_str());
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                              SWITCH_LOG_NOTICE, "connection failed: %s\n",
                              message);
          } break;
          case drachtio::AudioPipe::CONNECTION_DROPPED:
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                              SWITCH_LOG_NOTICE,
                              "connection dropped from far end\n");
            break;
...
switch_status_t fork_data_init(private_t *tech_pvt,
  ...
  if ((username = switch_channel_get_variable(
           channel, "MOD_AUDIO_BASIC_AUTH_USERNAME"))) {
    password =
        switch_channel_get_variable(channel, "MOD_AUDIO_BASIC_AUTH_PASSWORD");
  }
  ...
  if (bidirectional_audio_sample_rate > sampling) {
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                      "downscale_factor is %d\n", tech_pvt->downscale_factor);
  }
  tech_pvt->streamingPreBufSize =
      320 * tech_pvt->downscale_factor * 6;  // min 120ms prebuffer
  ...
  size_t buflen =
      LWS_PRE + (FRAME_SIZE_8000 * desiredSampling / 8000 * channels * 1000 /
                 RTP_PACKETIZATION_PERIOD * nAudioBufferSecs);

  // BUG-15 fix: use nothrow new so that allocation failure returns nullptr
  // instead of throwing
  drachtio::AudioPipe *ap = new (std::nothrow) drachtio::AudioPipe(
      tech_pvt->sessionId, host, port, path, sslFlags, buflen,
      read_impl.decoded_bytes_per_packet, username, password, bugname,
      bidirectional_audio_stream_enable, eventCallback);
  if (!ap) {
  ...
  if (desiredSampling != sampling) {
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                      "(%u) resampling from %u to %u\n", tech_pvt->id, sampling,
                      desiredSampling);
    tech_pvt->resampler = speex_resampler_init(
        channels, sampling, desiredSampling, SWITCH_RESAMPLE_QUALITY, &err);
    if (0 != err) {
      switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
  ...
  if (bidirectional_audio_sample_rate &&
      sampling != bidirectional_audio_sample_rate) {
    switch_log_printf(
        SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
        "(%u) bidirectional audio resampling from %u to %u, channels %d\n",
        tech_pvt->id, bidirectional_audio_sample_rate, sampling, channels);
    tech_pvt->bidirectional_audio_resampler =
        speex_resampler_init(1, bidirectional_audio_sample_rate, sampling,
...
void destroy_tech_pvt(private_t *tech_pvt) {
  ...
  if (tech_pvt->streamingPreBuffer) {
    CircularBuffer_t *cBuffer =
        (CircularBuffer_t *)tech_pvt->streamingPreBuffer;
    delete cBuffer;
...
int parse_ws_uri(switch_channel_t *channel, const char *szServerUri, char *host,
  ...
  if (0 == strncmp(server, "https://", 8) ||
      0 == strncmp(server, "HTTPS://", 8)) {
    *pSslFlags = flags;
    offset = 8;
    *pPort = 443;
  } else if (0 == strncmp(server, "wss://", 6) ||
             0 == strncmp(server, "WSS://", 6)) {
    *pSslFlags = flags;
    offset = 6;
    *pPort = 443;
  } else if (0 == strncmp(server, "http://", 7) ||
             0 == strncmp(server, "HTTP://", 7)) {
    offset = 7;
    *pSslFlags = 0;
    *pPort = 80;
  } else if (0 == strncmp(server, "ws://", 5) ||
             0 == strncmp(server, "WS://", 5)) {
    offset = 5;
    *pSslFlags = 0;
    *pPort = 80;
  } else {
  ...
  //- `([^/:]+)` captures the hostname/IP address, match any character except in
  //the set
  //- `:?([0-9]*)?` optionally captures a colon and the port number, if it's
  //present.
  ...
  if (std::regex_search(strHost, matches, re)) {
    ...
    for (int i = 0; i < matches.length(); i++) {
      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "parse_ws_uri -
    ...
    */
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                      "parse_ws_uri - invalid format %s\n", strHost.c_str());
    return 0;
  }
  switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                    "parse_ws_uri - host %s, path %s\n", host, path);
...
switch_status_t fork_session_init(
  ...
  private_t *tech_pvt =
      (private_t *)switch_core_session_alloc(session, sizeof(private_t));
  if (!tech_pvt) {
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
...
switch_status_t fork_session_cleanup(switch_core_session_t *session,
  ...
  if (pAudioPipe) {
    // BUG-35 fix: close() returns true when LWS thread will handle AudioPipe
    // cleanup (CONNECTED/CONNECTING states), false when caller should delete
    // (IDLE state)
...
switch_bool_t fork_frame(switch_core_session_t *session,
  ...
  if (switch_mutex_trylock(tech_pvt->mutex) == SWITCH_STATUS_SUCCESS) {
    ...
    drachtio::AudioPipe *pAudioPipe =
        static_cast<drachtio::AudioPipe *>(tech_pvt->pAudioPipe);
    if (pAudioPipe->getLwsState() !=
        drachtio::AudioPipe::LWS_CLIENT_CONNECTED) {
      switch_mutex_unlock(tech_pvt->mutex);
      return SWITCH_TRUE;
    }
    ...
    if (NULL == tech_pvt->resampler) {
      ...
      while (true) {
        ...
        if (available < pAudioPipe->binaryMinSpace()) {
          ...
          switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                            SWITCH_LOG_ERROR, "(%u) dropping packets!\n",
                            tech_pvt->id);
      ...
      while (switch_core_media_bug_read(bug, &frame, SWITCH_TRUE) ==
        ...
        if (frame.datalen) {
          spx_uint32_t out_len =
              available >> 1;  // space for samples which are 2 bytes
          spx_uint32_t in_len = frame.samples;

          speex_resampler_process_interleaved_int(
              tech_pvt->resampler, (const spx_int16_t *)frame.data,
              (spx_uint32_t *)&in_len,
              (spx_int16_t *)((char *)pAudioPipe->binaryWritePtr()), &out_len);
...
switch_bool_t dub_speech_frame(switch_media_bug_t *bug, private_t *tech_pvt) {
  ...
  if (rframe && rframe->datalen > 0) {
    ...
    int samplesToCopy =
        std::min(static_cast<int>(cBuffer->size()), samples_needed);
