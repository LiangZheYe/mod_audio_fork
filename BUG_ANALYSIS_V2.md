...
分析：`fork_session_connect` 永远返回 `SWITCH_STATUS_SUCCESS`，因为它只是调用 `AudioPipe::connect()` 将连接请求加入 pending 队列（`addPendingConnect`），是异步操作。连接真正的成功或失败由 LWS 线程异步回调处理（`LWS_CALLBACK_CLIENT_ESTABLISHED` / `LWS_CALLBACK_CLIENT_CONNECTION_ERROR`），通过 `eventCallback` 通知应用层。`start_capture` 第118行的 `if` 分支是死代码，永远不会进入。

真正的连接失败走的是 `eventCallback` → `CONNECT_FAIL` → `tech_pvt->pAudioPipe = nullptr` 路径，应用层收到 `mod_audio_fork::connect_failed` 事件后应调用 `stop` 清理。
...
### BUG-29: `fork_function` API 命令解析中 `assert(cmd)` 在生产环境无意义

**文件**: `mod_audio_fork.c:202`
**位置**: `fork_function` 入口
...
### BUG-30: `fork_function` 中 `argv` 数组大小为 10 但 `start` 命令需要 10 个参数（0-9 索引）

**文件**: `mod_audio_fork.c:194`
**位置**: API 命令解析
...
### BUG-31: `fork_function` 中 `start` 命令的 `parse_ws_uri` 失败后仍继续执行 `start_capture`

**文件**: `mod_audio_fork.c:319-326`
**位置**: start 命令处理
...
### BUG-32: `fork_session_connect` 无 NULL 检查直接解引用

**文件**: `lws_glue.cpp:728-733`
**位置**: `fork_session_connect` 函数
...
虽然正常流程下 `fork_session_connect` 在 `fork_session_init` 成功后调用，`pAudioPipe` 不应为 NULL。但如果 `fork_data_init` 中 `new (std::nothrow) AudioPipe` 失败返回了 NULL，`fork_data_init` 会返回 `SWITCH_STATUS_FALSE`，`fork_session_init` 会调用 `destroy_tech_pvt` 清理。然而 `destroy_tech_pvt` 现在会检查 `pAudioPipe` 是否为 NULL 再 delete，所以 init 失败路径是安全的。但防御性编程仍建议加检查。
...
当用户在连接还在 `CONNECTING` 状态时调用 stop，`close()` 直接 return 不做任何事。之后 `fork_session_cleanup` 设 `tech_pvt->pAudioPipe = nullptr` 并执行 `destroy_tech_pvt`。
...
在当前代码中，`LWS_CALLBACK_CLIENT_CLOSED` 先调用 `m_callback`（即 `eventCallback`），在 `eventCallback` 中设 `tech_pvt->pAudioPipe = nullptr`，然后再 `delete ap`。这个顺序是正确的。但 `eventCallback` 中 `CONNECTION_DROPPED` 分支不调用 `destroy_tech_pvt`，所以 `tech_pvt` 仍持有 `pAudioPipe = nullptr` 的状态，后续 `fork_session_cleanup` 中不会 double-delete。

这条确认：当前代码在 `CONNECTION_DROPPED`/`CLOSED_GRACEFULLY` 路径下，`delete ap` 不会导致 double-delete（因为 `eventCallback` 先设 nullptr，而 `fork_session_cleanup` 中 close 后也设 nullptr，`destroy_tech_pvt` 检查 nullptr 后跳过 delete）。
...
### BUG-44: `fork_session_stop_play` 使用 `trylock` 可能静默失败

**文件**: `lws_glue.cpp:1042-1048`
**位置**: `fork_session_stop_play` 函数
...
1. **AudioPipe 生命周期管理**: 当前 AudioPipe 的删除时机分布在三个地方（LWS 回调、`destroy_tech_pvt`、`fork_session_cleanup` 的 close 路径），建议统一由单一所有者管理。引入 `std::shared_ptr<AudioPipe>` + `std::weak_ptr<AudioPipe>` 可以优雅解决 double-delete 和 use-after-free 问题。
